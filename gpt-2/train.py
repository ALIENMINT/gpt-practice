...
class MLP(nn.Module):
    def __init__(self,config):
        super().__init__()
        self.c_fc  =nn.Linear(config.n_emb,4* config.n_emb,bias=False)
        self.gelu  =nn.GELU(approximate='tanh')
        self.c_proj=nn.Linear(4* config.n_emb,config.n_emb,bias=False)
    def forward(self,x):
...
class GPT(nn.Module):
    def __init__(self,config):
        ...
        self.transformer=nn.ModuleDict(dict(
            ...
            h=nn.ModuleList([Block(config) for _ in range(config.n_layer)]),#6
